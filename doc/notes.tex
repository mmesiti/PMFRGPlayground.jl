...
\begin{equation}
  ...
  i{(G(\omega,T,x))}^{-1}_{ab} = -\left[\left(\sqrt{T} \omega\right) \delta_{ab} +  i\Sigma^T_{ab}(x,\omega) \right]
\end{equation}
...
is
\begin{equation}
  S = -\left. \frac{\partial G}{\partial T} \right|_{\Sigma^T = const} ,
\end{equation}
...
\end{equation}
i.e., the derivative of this $G$ with respect to the flow parameter $\Lambda$, keeping $\Sigma$ constant.

So in this case we get
\begin{equation}
i S_{ab} = - \frac{2\Lambda}{\omega} iG_{ac} iG_{cb}
\end{equation}
Where we left understood the arguments $(\omega,T,x,\Lambda)$ both for functions $S_{ab}$ and $G_{ab}$.

And for the ``Katanin propagator'':
\begin{equation}
...
\begin{equation}
  \begin{aligned}
  \frac{d}{d\Lambda} \Gamma^{ii}_{abcd}(\omega_1,\omega_2,\omega_3,\omega_4) =
  T \sum_{\omega} \sum_k \left[ -\Gamma^{ik}_{aba'd'}(\omega_1,\omega_2,\omega,-s-\omega)
\Gamma^{ik}_{b'c'cd}(-\omega,\omega+s,\omega_3,\omega_4)P^{kk}_{a'b'c'd'}(-\omega,-s) \right. \\
    \left. \right]
  \end{aligned}
\end{equation}
...
\begin{equation}
  \frac{d}{d\Lambda} \Gamma^{ij}_{abcd}(\omega_1,\omega_2,\omega_3,\omega_4) = T \sum_{\omega} \sum_k
\end{equation}

Where
\begin{equation}
  P_{abcd}(i,j,\omega,\xi) = -iS_{kat,ab}(\omega,T,i)iG_{cd}(\omega+\xi,T,j)
\end{equation}
